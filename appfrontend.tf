...
resource "aws_s3_bucket" "frontend_bucket" {
  bucket = "terraform.stephitim.space"
}
...
resource "aws_cloudfront_origin_access_control" "frontend_oac" {
  name                              = "stephitim-space-oac"
  description                       = "OAC for terraform.stephitim.space S3 bucket access"
...
resource "aws_cloudfront_distribution" "frontend_cdn" {
  ...
  aliases = ["terraform.stephitim.space"]

  origin {
    origin_id                = "S3-terraform.stephitim.space"
  ...
  default_cache_behavior {
    target_origin_id       = "S3-terraform.stephitim.space"
...
# Create a DNS Alias record in Route53 pointing to CloudFront
